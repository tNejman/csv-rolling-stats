...
class ArgParser {
 ...
 public:
  ...
  static constexpr std::string_view HELP_MESSAGE =
      "Usage: <program-name> --in <input-filename> --out <output-filename>\n\n"
      "  -h, --help     Show this help message and exit.\n";

  [[nodiscard]] bool tryHandleHelpFlag() const noexcept {
  ...
  [[nodiscard]] std::pair<InputFilePath, OutputFilePath> getFileNames() const noexcept {
    constexpr int EXPECTED_ARG_COUNT_DEFAULT = 5;
    if ( concrete_arguments_.size() != EXPECTED_ARG_COUNT_DEFAULT ) {
      std::cout << "arg count does not match expected: " << EXPECTED_ARG_COUNT_DEFAULT << "\n\n" << HELP_MESSAGE;
      std::exit( exit_codes::FLAG_PARSE_ERROR );
    }
